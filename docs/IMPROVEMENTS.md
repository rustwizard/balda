...
### 13. 🔶 Matchmaking — отложено (решение принято)
**Файлы:** `cmd/server.go`, `internal/service/balda_service.go`, `internal/matchmaking/`

`matchmaking.Queue` создаётся, но `mm.Run(ctx)` в проде не вызывается и ни один хендлер не делает
`mm.Enqueue` — очередь сейчас живёт только в тестах. Пэйринг идёт по `Exp` (накопленный опыт из
`player_state`) с расширяющимся окном — это грубый прокси, а не ELO (`Exp` только растёт и не
зависит от силы соперника).

**Решение:** не подключать сейчас. MM нужен только для фичи «Быстрый бой» (авто-подбор), которой
во фронте нет; текущее лобби (создать игру / зайти в открытую из списка) закрывает потребность.
Подключать MM без quick-match-UX = добавить поверхность (эндпоинт + лайфцикл очереди + кнопка)
ради нуля видимой пользы. Код и тесты MM сохранены: механизм не зависит от источника рейтинга,
поэтому при появлении quick-match/ELO достаточно вызвать `go mm.Run`, добавить эндпоинт и
подменить `Exp` на рейтинг. Отложено до того момента.
...
**Затрагивает:** `internal/matchmaking/`, REST API, `internal/centrifugo/`, фронт

Одна главная кнопка «Играть» вместо «Создать игру»/«Играть с ботом»: сервер ищет
живого соперника по ELO (`matchmaking.Queue` уже написан и покрыт тестами, см. п. 13),
через 10–15 секунд безуспешного поиска — автоматически создаёт игру с ботом
(серверный дедлайн, не клиентский, чтобы не было гонки с найденным соперником).

Эндпоинты `POST /matchmaking/join|leave`, событие `match_found` в Centrifugo,
подключение `mm.Run(ctx)` в проде. Фронт: состояние поиска (спиннер + отмена).
«Игра с другом» (создание приватной игры + startapp-ссылка, п. 40) остаётся
вторичной опцией — виральный сценарий ломать нельзя.

Краевые случаи: отмена поиска, офлайн во время ожидания (чистить по presence),
две вкладки одного игрока, соперник нашёлся, но игрок ушёл.
