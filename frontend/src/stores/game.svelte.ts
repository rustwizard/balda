...
export function createGameState() {
    ...
    let notif = $state<{ message: string; kind: "error" | "warn" } | null>(
        null,
    );
    ...
    function mergePlayerState(p: PlayerGameState): PlayerInfo {
        ...
        return {
            ...
            nickname:
                existing?.nickname ||
                (p.uid === playerUid ? nickname : "Соперник"),
            exp: p.exp ?? existing?.exp ?? 0,
            rating: p.rating ?? existing?.rating ?? 0,
            ratingGained: p.rating_gained ?? 0,
    ...
    function applySkipWarn(ev: EvSkipWarn) {
        players = players.map((p) =>
            p.uid === ev.player_uid
                ? { ...p, consecutiveSkips: ev.skips_used }
                : p,
        );
    }
    ...
    function selectCell(row: number, col: number) {
        const idx = selectedPath.findIndex(
